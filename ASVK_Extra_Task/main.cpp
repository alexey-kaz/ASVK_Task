...
int mainAlgorithm ( Proc &processors, Prog &programs, int * x, int &F_best, int times ) {
    ...
    #pragma omp parallel
    while (z < 1000) { //using while because in OMP "for" number of iterations can't be changed mid-loop
        for ( int i = 0; i < programs.getProgNum (); i++ ) // randomly generate current vector values
            x[i] = rand () % processors.getProcNum ();
...
#pragma omp critical // to make sure iterations in all threads are increased
            iterations++;
            z++;
            continue;
        } else if ( F_curr < F_best ) { // new network usage is better that the best result so far
#pragma omp critical // to make sure F_best is changed in all threads at once
...
int main ( int argc, char **argv ) {
    ...
    if (argc == 3) {
        ...
        if ( result && ( file.child ( "Task" ) ) ) {// check if the file loads correctly & first node is Task
            ...
            iterations = mainAlgorithm ( processors, programs, x, f_Best, times );
        ...
        cout << "either no input path or thread number specified in argv" << endl;
    }
    return 0;
}
